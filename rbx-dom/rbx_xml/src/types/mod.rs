    ...
    { $($variant_name: ident : $inner_type: ty,)* } => {
        ...
        pub fn read_value_xml<R: Read>(
            ...
            match xml_type_name {
                ...
                "int64" => Ok(Some(Variant::Int64(i64::read_outer_xml(reader)?))),
        ...
        pub fn write_value_xml<W: Write>(
            ...
            match value {
                Variant::Int64(value) => {
                    let converted = match i32::try_from(*value) {
                        Ok(v) => v,
                        Err(_) => {
                            println!("[legacy_place::convert] failed to convert int64 to int32 on property {} as exceeds i32 range: {}, defaulting to -1 (shouldn't cause any issues)", xml_property_name, value);
                            -1
                        }
                    };

                    converted.write_outer_xml(xml_property_name, writer)
                }
