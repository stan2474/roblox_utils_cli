...
use rbx_dom_weak::{
    types::{Ref, Color3, SharedString, SharedStringHash, Variant, VariantType},
    WeakDom,
};
...
fn serialize_instance<'dom, W: Write>(
    ...
    for (property_name, value) in property_buffer.drain(..) {
        ...
        if let Some(serialized_descriptor) = maybe_serialized_descriptor {
            ...
            let changed_type = if data_type == VariantType::Color3uint8 {
                VariantType::Color3
            } else {
                data_type
            };
            ...
            let mut converted_value = match value.try_convert_ref(instance.class, changed_type) {
                Ok(value) => value,
                Err(message) => {
                    return Err(
            ...
            converted_value = normalize_color3uint8(converted_value);

            if serialized_name == "Color3uint8" {
                serialized_name = "Color";
            }
            write_value_xml(writer, state, serialized_name, converted_value.as_ref())?;
        } else {
            match state.options.property_behavior {
                ...
                EncodePropertyBehavior::WriteUnknown | EncodePropertyBehavior::NoReflection => {
                    ...
                    let val = normalize_color3uint8(Cow::Borrowed(value));
                    let property_xml_name = if property_name == "Color3uint8" {
                        "Color"
                    } else {
                        property_name
                    };
                    write_value_xml(writer, state, property_xml_name, val.as_ref())?;
                }
...
fn normalize_color3uint8<'a>(value: Cow<'a, Variant>) -> Cow<'a, Variant> {
    match value {
        Cow::Borrowed(variant) => match variant {
            Variant::Color3uint8(color) => Cow::Owned(Variant::Color3(Color3::from(*color))),
            _ => Cow::Borrowed(variant),
        },
        Cow::Owned(variant) => match variant {
            Variant::Color3uint8(color) => Cow::Owned(Variant::Color3(Color3::from(color))),
