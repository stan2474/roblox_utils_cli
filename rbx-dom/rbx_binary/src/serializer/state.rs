...
impl<'dom, 'db, W: Write> SerializerState<'dom, 'db, W> {
    ...
    pub fn collect_type_info(&mut self, instance: &'dom Instance) -> Result<(), InnerError> {
        ...
        for (prop_name, prop_value) in &instance.properties {
            ...
            let canonical_name;
            let mut serialized_name;
            let mut serialized_ty;
            ...
            if serialized_name == "Color3uint8" {
                serialized_name = "Color".into();
            }

            if serialized_ty == VariantType::Color3uint8 {
                serialized_ty = VariantType::Color3;
            }
        ...
        for (type_name, type_info) in &self.type_infos.values {
            for (prop_name, prop_info) in &type_info.properties {
                ...
                match prop_info.prop_type {
                    ...
                    Type::Int32 => {
                        ...
                        for (i, rbx_value) in values {
                            match rbx_value.as_ref() {
                                ...
                                Variant::Int64(value) => {
                                    let v = *value as i64;
                                    let converted: i32 = if v < i32::MIN as i64 || v > i32::MAX as i64 {
                                        println!("[legacy_place::convert] failed to convert int64 to int32 on property {} as exceeds i32 range: {}, defaulting to -1 (shouldn't cause any issues)", prop_name, value);
                                        -1
                                    } else {
                                        v as i32
                                    };
                                ...
                                _ => return type_mismatch(i, &rbx_value, "Int32 test"),
                            }
                        }
                        ...
                        for (i, rbx_value) in values {
                            match rbx_value.as_ref() {
                                ...
                                Variant::Color3uint8(value) => {
                                    let color: Color3 = (*value).into();
                                    r.push(color.r);
                                    g.push(color.g);
                                    b.push(color.b);
                                }
                                _ => return type_mismatch(i, &rbx_value, "Color3 or Color3uint8"),
                            }
                        }
                    ...
                    Type::Color3uint8 => unreachable!(), // color3uint8 should get automatically converted to color3, if not, something's very wrong
                    Type::Int64 => unreachable!(), // same with int64 (should convert to int32)
                    Type::SharedString => {
                    ...
                    Type::Content => {
                        ...
                        for (i, rbx_value) in values {
                            let rbx_value = match rbx_value {
                                Cow::Borrowed(value) => {
                                    if let Variant::ContentId(content_id) = value {
                                        let content = if content_id.as_str().is_empty() {
                                            Content::none()
                                        } else {
                                            Content::from_uri(content_id.as_str())
                                        };

                                        Cow::Owned(Variant::Content(content))
                                    } else {
                                        Cow::Borrowed(value)
                                    }
                                }
                                Cow::Owned(value) => match value {
                                    Variant::ContentId(content_id) => {
                                        let uri = content_id.into_string();
                            ...
                            if let Variant::Content(content) = rbx_value.as_ref() {
                                source_types.push(match content.value() {
                                    ...
                                    _ => return Err(invalid_value(i, rbx_value.as_ref())),
                                });
                            } else {
                                return type_mismatch(i, rbx_value.as_ref(), "Content");
                            }
                        }
